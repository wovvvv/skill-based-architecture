...
assert_contains "$plan_knowledge_closure" "read each Component from its canonical Requirement and Plan owners" "Plan/Knowledge reconciliation owner split"
assert_contains "$plan_knowledge_closure" 'A `done` Component Plan is frozen' "Plan/Knowledge frozen Component Plan lifecycle"
assert_contains "$plan_knowledge_closure" 'no required Component Plan may remain `draft` or `executing`' "Plan/Knowledge unfinished Component Plan lifecycle gate"
...
assert_contains "$plan_knowledge_closure" "an independent Component Plan retains its own lifecycle" "Plan/Knowledge independent Component Plan lifecycle"
assert_contains "$plan_knowledge_closure" "Component Requirement's Decision Delta, boundary, and acceptance" "Plan/Knowledge Requirement-owned reconciliation payload"
assert_contains "$plan_knowledge_closure" "Component Plan's technical decisions, fresh proof, delivery responsibility, and durable technical distillation" "Plan/Knowledge Plan-owned reconciliation payload"
...
assert_contains "$plan_knowledge_closure" "integrated result satisfies the common governing Requirement projection and its acceptance" "Plan/Knowledge Requirement-governed integrated acceptance"
assert_contains "$plan_knowledge_closure" "Integrating Plan supplies only consumed technical interfaces, ordering, proof, and delivery responsibility" "Plan/Knowledge technical integration responsibility"
assert_contains "$plan_knowledge_closure" "cannot create or revise the common outcome or integrated acceptance" "Plan/Knowledge cannot invent integrated acceptance"
...
assert_contains "$external_delivery" "current explicit user request and the Governing Requirement, as projected into the active Task Anchor" "external delivery Requirement-owned artifact set"
assert_contains "$external_delivery" "Plans may attach only an already-bound item's technical identity" "external delivery Plan technical identity boundary"
assert_contains "$external_delivery" "Plan cannot add, omit, narrow, or modify an artifact or requested terminal state" "external delivery rejects Plan-owned target changes"
assert_not_contains "$external_delivery" "active Task Anchor, and every governing Plan" "external delivery rejects former Plan authority"
...
assert_contains "$plan_knowledge_closure" "read each Component from its canonical Requirement and Plan owners" "closure reads Component truth at separate owners"
...
assert_contains "$plan_feature" "without mirroring Component content" "composition rejects durable content mirrors"
assert_contains "$plan_feature" "Requirement meaning or acceptance, or live status" "composition rejects Requirement and status mirrors"
...
assert_contains "$task_execution" "Task Execution first projects that Requirement into exactly one Integrating Task Anchor" "task execution owns Integrating Anchor projection"
assert_contains "$task_execution" 'Only after `implementation-ready` does Task Execution derive one Integrating Native Plan' "task execution owns Integrating Native Plan derivation"
...
assert_contains "$composite_execution" "read its governing Requirement or Requirement projection for Goal, meaning, invariant, boundary, and acceptance" "Component normative truth comes from Requirement"
assert_contains "$composite_execution" "Read its Component Plan only for technical decisions" "Component Plan is technical-only"
assert_contains "$composite_execution" "conflict returns to Requirement Definition and the affected Requirement owner" "Component normative conflict returns to Requirement"
assert_contains "$composite_execution" "technical interface, adapter, ordering, risk, proof, or delivery conflict returns" "Component technical conflict returns to Plan"
assert_contains "$composite_execution" "this protocol neither creates that Anchor nor defines the common outcome or integrated acceptance" "composite protocol cannot create Anchor or acceptance"
assert_contains "$composite_execution" "a frozen Component Plan changes only through a technical successor" "frozen Component Plan conflict creates a technical successor"
assert_not_contains "$composite_execution" "multiple independently authoritative Plans" "composite execution rejects Plan normative authority"
assert_not_contains "$composite_execution" "Component meaning, invariant, boundary, accepted interface, or local acceptance returns to that Component's Design owner" "composite execution rejects normative Design-owner return"
assert_not_contains "$composite_execution" "one Integrating Task Anchor and one Native Plan cover the common outcome and integrated acceptance" "composite execution cannot synthesize normative Anchor"
...
assert_contains "$plan_feature" "rule-update/business-truth.md" "Requirement implementation handoff enters Business Truth directly"
assert_contains "$business_profile" "rule-update/business-truth.md" "standalone modeling enters Business Truth directly"
assert_not_contains "$plan_feature" '[`update-rules.md`](update-rules.md)' "Requirement implementation handoff does not redispatch through update-rules"
assert_contains "$update_rules" "frozen Requirement provenance" "Rule Update separates Requirement provenance"
assert_contains "$update_rules" "frozen Plan technical provenance" "Rule Update separates Plan technical provenance"
assert_not_contains "$update_rules" "frozen Plan provenance" "Rule Update rejects ambiguous Plan provenance"
assert_contains "$business_truth" "Requirement implementation handoff" "Business Truth names Requirement source"
assert_not_contains "$business_truth" "from Plan handoff" "Business Truth rejects Plan-owned desired meaning"
