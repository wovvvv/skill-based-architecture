...
- After the brainstorm feedback, inspect real evidence (README, build files, CI, entry points, configs, module layout, tests) and classify conclusions as `Confirmed from code/config`, `User-confirmed desired truth`, `Inferred`, or `Unknown`. Surface conflicts; current code may expose constraints/cost/risk but cannot silently rewrite the user-confirmed mainline.
