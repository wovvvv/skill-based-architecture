...
Capture source before synthesis. A clear user statement needs no rewrite: keep it verbatim as `User source`. If readability truly requires normalization, apply only punctuation, obvious typo, grammar, or word-order edits and keep the original wording adjacent; do not omit or merge a command/question, acceptance/refusal, actor/ownership, timing/commitment, uncertainty/emphasis, boundary, reason, or rejected direction. Any `Agent-derived interpretation` is optional, explicitly labeled, and subordinate; it may explain consequences but cannot replace the source as the only normative record. Apply [`maintain-docs.md` § Source-Preserving User Input](maintain-docs.md#source-preserving-user-input) before a user message becomes a compressed Decision Context.

### Semantic Fidelity Before Plan Compression

When a Plan proposes compressing, summarizing, deduplicating, moving, or deleting durable instructions or user-brainstorm meaning, consume [`maintain-docs.md` § Semantic Before/After Reconciliation](maintain-docs.md#step-5-semantic-beforeafter-reconciliation) before marking the proposal confirmed or handing it to implementation. The minimum unit is a decision-changing clause, not a physical line or paragraph. For conversational input, first preserve the source; keep a short answer bound to its original question/options, classify each statement as `proposed`, `confirmed`, `superseded`, or evidence-needed, and inventory speech act/force, actor/ownership, timing/commitment, uncertainty/emphasis, definition/identity, applicability/preconditions, boundaries/counterexamples, negations/rejected directions, reasons/consequences, authority, and scope before writing an optional derived summary.

Each source clause must have one disposition: preserve verbatim, faithful rewrite, move to an exact owner/section, or explicitly delete because its premise no longer applies. A move or summary is equivalent only when meaning, applicability/authority, and normal activation timing are all the same. If a fresh Agent could make a different decision from the result, retain the source wording and return the Plan for clarification; shorter or structurally green is not semantic proof.
...
For two or more dependent tasks, declare `Files`, `Consumes`, `Produces`, and observable `Acceptance`; omit placeholder decomposition. Before implementation handoff, reconcile authority, Current/Target, impact, risks, proof, Open Questions, artifact links, and task interfaces. Run Plan Mainline Handoff: replay confirmed/superseded decisions; for business-bearing work, distilled confirmed business meaning goes directly through [`workflows/rule-update/business-truth.md` § Plan Decision Source](rule-update/business-truth.md#plan-decision-source); initialize truthful `distilled_to:` targets; retain the source Plan path; and pass its chosen outcome, acceptance criteria, boundaries, and task breakdown to [`task-execution.md`](task-execution.md). For an Integrating Plan, every consumed interface must be stable and it becomes the only source for the Native Plan; Component reconciliation precedes integrated `done`. The dossier owns decisions, not live step status.
