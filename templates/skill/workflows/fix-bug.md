...
A Design Change leaves this workflow for an approved Plan. For insufficient context, search evidence first and cross-check it against any existing user-confirmed answer. If they align and already establish expected behavior, continue without another question. Otherwise trace the smallest concrete business flow and form an evidence-backed working conclusion; then use [`plan-feature.md` § Question Gate](plan-feature.md#question-gate) to ask only an unresolved ambiguity that could change repair classification or expected behavior, not request exhaustive detail or an abstract definition from scratch. A completely absent model is created only if the user chooses “now”. Obvious technical failures need no business model.
