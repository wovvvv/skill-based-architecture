...
1. **Read back the contract** — restate the governing Requirement projection and Task Anchor (or Simple-task outcome): matched route, Goal-level acceptance evidence, material Boundaries, Plan-selected proof set, escalation signals, stop point, forbidden shortcuts, every governing Requirement and Plan path, and when activated the final expanded [`Change Contract`](../protocol-blocks/change-contract.md). Closure consumes that contract; it does not redefine Requirement meaning, source-localization stages, read breadth, or proof depth after green. After a long/interrupted task, use `protocol-blocks/reboot-check.md`.
...
3. **Verify with fresh, fitted evidence**:
   ...
   When an adopted `tests-as-spec` contract is active, report the recorded red reproduction → same-case green proof and any remaining regression uncertainty; historical red evidence remains valid and is not a reason to rewind or rerun the broken version.
...
6. **Run conditional integrity work**:
   - durable instruction or user-meaning compression, summary, deduplication, move, or deletion -> consume [`maintain-docs.md` § Semantic Before/After Reconciliation](maintain-docs.md#step-5-semantic-beforeafter-reconciliation) first; prove every decision-changing clause's meaning, applicability/authority, status, and normal activation timing before running structural checks;
