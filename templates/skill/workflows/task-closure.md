...
5. **Run the AAR below.** Any yes enters `update-rules.md`; all no stops recording. If recording changes any deliverable, return to the affected verification and integrity checks before declaring readiness.
...
7. **Declare readiness** — only after Steps 1–6 pass, name the already-authorized delivery action, expected artifact, and exact verification. `Ready for Delivery` is a checkpoint, not task completion.
