# Source Localization Protocol

Use this after the selected workflow needs a current implementation fact but cannot name a sufficiently evidenced owner, source of truth, path, symbol, or change point. This file owns the complete technical narrowing contract; it never decides desired product meaning.

## Trigger And Runtime State
- Enter when technical ownership is unresolved; skip directly to a selected-source check when the target is already proven concrete.
- Re-enter when a premise fails, several owners survive, a generated/shared boundary appears, or tracing contradicts the candidate.
...
5. **Confirm** — prove the canonical owner/change point, affected semantic path, next action/proof, and residual uncertainty.

Each stage names its unresolved question, fitted evidence, output consumed next, and advance/return condition. Physical files may grow along a proven semantic path, but uncertainty must narrow; a path, lexical match, file read, or successful command is not owner proof.

## Tool And Exposure Boundary
- Stages 1-2 expose no source bodies; Stage 3 returns bounded candidates; Stage 4 is the first selected-source read.
- Empty/noisy/truncated results refine or return instead of proving absence or triggering whole-repository reading.
- Derived/generated targets return to their canonical producer; persisted technical maps are optional, source-subordinate accelerators owned by a separate recording workflow.
