...
5. For every answer, bind every short answer to its original question/options and preserve speech act/force, actor/ownership, timing/commitment, uncertainty/emphasis, definition/identity, applicability/preconditions, boundary/counterexample, negation/rejected direction, rationale/consequence, authority, scope, and `proposed | confirmed | superseded` status. Record that source-faithful answer through [`define-requirement.md` § Decision Authority And Source Fidelity](../workflows/define-requirement.md#decision-authority-and-source-fidelity) in the governing Requirement mainline. If meaning changes, mark the prior decision `superseded`; never silently rewrite it. When faithful normalization is uncertain, retain the source wording or ask the user rather than inventing a cleaner interpretation.
6. Reconcile any routed business rule, affected Requirement Contract, Plan/Component interface, Task Anchor, acceptance criteria, and implementation evidence before resuming. If the answer changes the mainline, re-project the Task Anchor, rebind implementation facts, and replace the remaining Native Plan before more mutation.
