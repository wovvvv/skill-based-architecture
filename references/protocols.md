...
Task Execution sits after route selection and before Task Closure. One clear action with one direct check stays Simple and pays no planning cost. Other tasks establish Task Anchor state: one observable Goal, Done When evidence, and material Boundaries only when present. That state is not a fixed chat template. Default Managed work uses a short natural-language alignment only when it helps the user verify direction; a visible native Plan owns step display and is not repeated in chat. Long, complex, scope-sensitive, confirmation-dependent, or no-native-Plan work may use one complete structured task brief. Design-derived work additionally carries the source Plan path and relevant user-confirmed Decision Context into every Plan edit, code reasoning/implementation step, review, and closure. Before each main step and after correction, failed/surprising evidence, Subagent return, or interruption, a compact Anchor Checkpoint brings Goal, remaining Done When evidence, current-step output/check, relevant Boundaries, and applicable mainline decisions back into current attention. Code may expose constraint/cost/risk but cannot rewrite that mainline: on material drift, pause the affected path and ask the user immediately rather than silently compromising or deferring disclosure. Before verification, bind material risk to fitted evidence and a stop/escalation condition; stop when that contract is satisfied rather than treating test count as evidence quality. Runtime step recitation remains Session state; the source Plan is the durable decision record, not a substitute for Native Plan status.
